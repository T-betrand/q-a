...
def like_view(request, pk):
    ...
    if post.likes.filter(id=request.user.id).exists():
        ...
        liked = True
    return HttpResponseRedirect(reverse('stackcore:question-detail', args=[str(pk)]))
...
class CommentDetailView(CreateView):
    ...
    template_name = 'stackcore/question-detail.html'
    
    def form_valid(self, form):
        form.instance.question_id = self.kwargs['pk']
        return super().form_valid(form)
    success_url = reverse_lazy('stackcore:question-detail')

class AddCommentView(CreateView):
    model = Comment
    form_class = CommentForm
    
    template_name = 'stackcore/question-answer.html'

    def form_valid(self, form):
        form.instance.question_id = self.kwargs['pk']
        return super().form_valid(form)
    success_url = reverse_lazy('stackcore:question-lists')